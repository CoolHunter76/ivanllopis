...
def test_work_life_routes_render_in_all_languages():
    for language in SUPPORTED:
        response = client.get(f"/{language}/work-life")

        assert response.status_code == 200
        assert "career-list" in response.text
        assert "career-entry" in response.text
        assert "HP Hewlett-Packard" in response.text
        assert "Repsol" in response.text
        assert "MAPFRE" in response.text
        assert "ADIF" in response.text
        assert "EnergyaVM" in response.text
